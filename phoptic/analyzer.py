...
from phoptic.fitting.fitters import CrossAndPowerSpectrumFitter, get_components
from phoptic.fitting.statistics import reduced_chisqd
...
from phoptic.timing.fourier import AveragedCrossSpectrum, AveragedPowerSpectrum, CrossSpectrum, PowerSpectrum
...
class Analyzer:
    ...
    def power_spectra(
        ...
        scale: Literal['linear', 'semilogx', 'semilogy', 'loglog'] = 'linear',
        save: bool = False,
        norm: Literal['frac'] = 'frac',
        return_fig: bool = False,
        ) -> dict[str, PowerSpectrum] | tuple[dict[str, PowerSpectrum], Figure]:
        """
        Compute the power spectra of the light curves.
        
        Parameters
        ----------
        scale : Literal["linear", "semilogx", "semilogy", "loglog"], optional
            The plot scale, by default 'linear'.
        save : bool, optional
            Whether to save the resulting plot, by default `False`.
        norm : Literal["frac"], optional
            The power spectrum normalisation, by default 'frac'.
        return_fig : bool, optional
            Whether to return the resulting plot, by default `False`.
        
        Returns
        -------
        dict[str, PowerSpectrum] | tuple[dict[str, PowerSpectrum], Figure]
            The power spectra and optionally the resulting figure.
        """
        ...
        ps: dict[str, PowerSpectrum] = {}
        
        for i, key in enumerate(self.keys):
            ps[key] = PowerSpectrum.from_timeseries(
                ts=self.light_curves,
                signal_column_name=f'{key}_rel_flux',
                norm=norm,
                )
            
            axes[i].step(
                ps[key].freqs.to_value(u.Hz),
                ps[key].powers,
                c='k',
                lw=1,
                where='mid',
        ...
        if save:
            save_figure(
                fig=fig,
                path=self.out_directory.joinpath(f'plots/{self.prefix}_power_spectra.pdf'),
                )
    ...
    def averaged_power_spectra(
        self,
        segment_size: Quantity,
        rebin_log: bool = False,
        rebin_f: float = 0.01,
        scale: Literal['linear', 'semilogx', 'semilogy', 'loglog'] = 'linear',
        save: bool = False,
        norm: Literal['frac'] = 'frac',
        return_fig: bool = False,
        ) -> dict[str, AveragedPowerSpectrum] | tuple[dict[str, AveragedPowerSpectrum], Figure]:
        """
        Compute the averaged power spectra of the light curves.
        
        Parameters
        ----------
        segment_size : Quantity
            The size of the segments used to compute the averaged power spectra.
        rebin_log : bool, optional
            Whether to rebin the power spectra logarithmically, by default `False`.
        rebin_f : float, optional
            The logarithmic rebinning factor, by default 0.01. Only used if `rebin_log=True`.
        scale : Literal["linear", "semilogx", "semilogy", "loglog"], optional
            The plot scale, by default 'linear'.
        save : bool, optional
            Whether to save the resulting plot, by default `False`.
        ...
        -------
        dict[str, AveragedPowerSpectrum] | tuple[dict[str, AveragedPowerSpectrum], Figure]
            The averaged power spectra and optionally the resulting figure.
        """
        ...
        ps: dict[str, AveragedPowerSpectrum] = {}
        
        for i, key in enumerate(self.keys):
            new_ps = AveragedPowerSpectrum.from_timeseries(
                ts=self.light_curves,
                signal_column_name=f'{key}_rel_flux',
                norm=norm,
            ...
            if rebin_log:
                ps[key] = new_ps.rebin_log(f=rebin_f)
            else:
                ps[key] = new_ps
        ...
        fig.supylabel(f'Power [{ps[key].get_norm_units()}]', fontsize='large')
        axes[-1].set_xlabel('Frequency [Hz]', fontsize='large')
        ...
        if save:
            save_figure(
                fig=fig,
                path=self.out_directory.joinpath(f'plots/{self.prefix}_averaged_power_spectra.pdf'),
                )
    ...
    def averaged_cross_spectra(
        ...
        norm: Literal['frac'] = 'frac',
        return_fig: bool = False,
        ) -> dict[str, AveragedCrossSpectrum] | tuple[dict[str, AveragedCrossSpectrum], Figure]:
        """
        Compute the averaged cross spectra of the light curves.
        
        Parameters
        ----------
        segment_size : Quantity
            The size of the segments used to compute the averaged cross spectra.
        rebin_log : bool, optional
            Whether to rebin the cross spectra logarithmically, by default `False`.
        rebin_f : float, optional
            The logarithmic rebinning factor, by default 0.01. Only used if `rebin_log=True`.
        scale : Literal["linear", "semilogx", "semilogy", "loglog"], optional
            The plot scale, by default 'linear'.
        save : bool, optional
            Whether to save the resulting plot, by default `False`.
        norm : Literal["frac"], optional
            The cross spectrum normalisation, by default 'frac'.
        return_fig : bool, optional
            Whether to return the resulting plot, by default `False`.
        ...
        -------
        dict[str, AveragedCrossSpectrum] | tuple[dict[str, AveragedCrossSpectrum], Figure]
            The averaged cross spectra and optionally the resulting figure.
        """
        
        assert len(self.keys) > 1, '[OPTICAM] Cannot compute cross spectrum from a single band.'
        
        cs: dict[str, AveragedCrossSpectrum] = {}
        ...
        for key in self.keys:
            for key2 in self.keys:
                ...
                if f'{key2} x {key}' in cs.keys():
                    continue
                
                new_cs = AveragedCrossSpectrum.from_timeseries(
                    ts=self.light_curves,
                    signal_column_name=f'{key}_rel_flux',
                    signal_column_name2=f'{key2}_rel_flux',
                    norm=norm,
                    segment_size=segment_size,
                    )
                
                new_key = f'{key} x {key2}'
                if rebin_log:
                    cs[new_key] = new_cs.rebin_log(f=rebin_f)
                else:
                    cs[new_key] = new_cs
        ...
        for i, key in enumerate(cs.keys()):
            axes[i].step(
                cs[key].freqs.to_value(u.Hz),
                np.abs(cs[key].powers * np.conj(cs[key].powers)),
                c='k',
                lw=1,
                where='mid',
                )
            ...
            axes[i].errorbar(
                cs[key].freqs.to_value(u.Hz),
                np.abs(cs[key].powers * np.conj(cs[key].powers)),
                np.abs(cs[key].errs * np.conj(cs[key].errs)),
                fmt='k.',
                ms=2,
                ecolor='grey',
                elinewidth=1,
        ...
        fig.supylabel(f'Cross spectral amplitude [{cs[key].get_norm_units()}]', fontsize='large')
        axes[-1].set_xlabel('Frequency [Hz]', fontsize='large')
        
        if save:
            save_figure(
                fig=fig,
                path=self.out_directory.joinpath(f'plots/{self.prefix}_averaged_cross_spectra.pdf'),
                )
    ...
    def fit_multiple_spectra(
        ...
        return_fig: bool = False,
        **kwargs,
        ) -> tuple[Model, Model, Model, Model] | tuple[Model, Model, Model, Model, Figure]:
        
        # fit model
        fitter = CrossAndPowerSpectrumFitter()
        results, fitted_models = fitter(model, cs=cs, **kwargs)
        fitted_ref_ps_model, fitted_sub_ps_model, fitted_re_cs_model, fitted_im_cs_model = fitted_models
        
        # get Poisson noise levels of power spectra
        for comp in fitted_ref_ps_model:
            if comp.__class__.name == 'Noise':
                ref_P_noise = comp.amplitude
        for comp in fitted_sub_ps_model:
            if comp.__class__.name == 'Noise':
                sub_P_noise = comp.amplitude
        
        print(ref_P_noise)
        print(sub_P_noise)
        
        cs_model = fitted_re_cs_model(cs.freqs) + 1j * fitted_im_cs_model(cs.freqs)
        ...
        # add axes to figure
        # we use two nested for loops and then reshape the axes array to keep the indexing consistent
        # with plt.subplots()
        ...
        axes = np.asarray(fig.get_axes()).reshape(nrows, ncols)
        ...
        def plot_psd(
            ...
            dof: int = len(y) - len(model_to_fit_params(model)[0])  # number of degrees of freedom
            
            ax.set_title(title, fontsize='large')
            ax.set_ylabel(y_label, fontsize='large')
            
            ax.errorbar(x, x * (y - Pnoise), x * yerr, ls='none', marker='.', ms=2, ecolor='grey', elinewidth=1, zorder=0, c='k')
            ax.step(x, x * (model(x) - Pnoise), c='r', lw=1, where='mid', label=f'$\\chi_\\nu^2 = {reduced_chisqd(exp=model(x), obs=y, err=yerr, dof=dof):.2f}$')
            
            for comp in model:
                # skip Poisson noise component
                if comp.__class__.name == 'Noise':
                    print('noise')
                    continue
                ax.step(x, x * comp(x), ls=':', c='grey', lw=1, where='mid', zorder=0)
            
            model_y = model(x) - Pnoise
            lo = np.min(x * model_y).value
            hi = np.max(x * model_y).value
            ...
            r = (y - model(x)) / yerr
            r_ax.errorbar(x, r, np.ones_like(r), ls='none', marker='.', ms=2, ecolor='grey', elinewidth=1, zorder=0, c='k')
            r_ax.axhline(0, c='r', lw=1)
            
            r_ax.set_xlabel('Frequency [Hz]', fontsize='large')
            r_ax.set_ylabel('$\\Delta \\chi$', fontsize='large')
        ...
        def plot_cs(
            ...
            dof: int = len(cs.freqs) - len(model_to_fit_params(re_model)[0])  # degrees of freedom (im model has no free params so only re model contributes)
            
            pos_ax.set_title('CS [Re sin($\\pi$/4) + Im cos($\\pi$/4)]', fontsize='large')
            neg_ax.set_title('CS [Re cos($\\pi$/4) - Im sin($\\pi$/4)]', fontsize='large')
            pos_ax.set_ylabel('$\\nu P_\\nu$', fontsize='large')
            neg_ax.set_ylabel('$\\nu P_\\nu$', fontsize='large')
            
            pos_model = re_model(cs.freqs) * np.sin(theta) + im_model(cs.freqs) * np.cos(theta)
            neg_model = re_model(cs.freqs) * np.cos(theta) - im_model(cs.freqs) * np.sin(theta)
            
            pos_ax.errorbar(cs.freqs, cs.freqs * cs.positive, cs.freqs * cs.positive_errs, ls='none', marker='.', ms=2, ecolor='grey', elinewidth=1, zorder=0, c='k')
            pos_ax.step(cs.freqs, cs.freqs * pos_model, c='r', lw=1, where='mid', label=f'$\\chi_\\nu^2 = {reduced_chisqd(exp=pos_model, obs=cs.positive, err=cs.positive_errs, dof=dof):.2f}$')
            
            neg_ax.errorbar(cs.freqs, cs.freqs * cs.negative, cs.freqs * cs.negative_errs, ls='none', marker='.', ms=2, ecolor='grey', elinewidth=1, zorder=0, c='k')
            neg_ax.step(cs.freqs, cs.freqs * neg_model, c='r', lw=1, where='mid', label=f'$\\chi_\\nu^2 = {reduced_chisqd(exp=neg_model, obs=cs.negative, err=cs.negative_errs, dof=dof):.2f}$')
            
            for re_comp, im_comp in zip(get_components(re_model), get_components(im_model)):
                
                pos_comp = re_comp(cs.freqs) * np.sin(theta) + im_comp(cs.freqs) * np.cos(theta)
                neg_comp = re_comp(cs.freqs) * np.cos(theta) - im_comp(cs.freqs) * np.sin(theta)
                
                pos_ax.step(cs.freqs, cs.freqs * pos_comp, ls=':', c='grey', lw=1, where='mid', zorder=0)
                neg_ax.step(cs.freqs, cs.freqs * neg_comp, ls=':', c='grey', lw=1, where='mid', zorder=0)
            
            for y, err, model, ax, r_ax in zip([cs.positive, cs.negative], [cs.positive_errs, cs.negative_errs], [pos_model, neg_model], [pos_ax, neg_ax], [pos_r_ax, neg_r_ax]):
                lo = np.min(cs.freqs * model).value
                hi = np.max(cs.freqs * model).value
                ax.set_ylim(lo / 2, hi * 2)
                ax.legend(
                    handlelength=0,
                    fontsize='large',
                ...
                r = (y - pos_model) / err
                r_ax.errorbar(cs.freqs, r, np.ones_like(r), ls='none', marker='.', ms=2, ecolor='grey', elinewidth=1, zorder=0, c='k')
                r_ax.axhline(0, c='r', lw=1)
        ...
        plot_psd(
            ax=axes[2, 0],
            title='Subject power spectrum',
            x=cs.freqs,
            y=cs.sub_ps.powers,
            yerr=cs.sub_ps.errs,
            model=fitted_sub_ps_model,
            y_label='$\\nu P_\\nu$',
            scale='semilogy',
            r_ax=axes[3, 0],
            Pnoise=sub_P_noise
        ...
        plot_cs(
            ...
            neg_r_ax=axes[3, 1],
            cs=cs,
            re_model=fitted_re_cs_model,
            im_model=fitted_im_cs_model,
            )
        
        # compute coherence
        coh, coh_err = cs.coherence(
            P_noise_ref=ref_P_noise,
            P_noise_sub=sub_P_noise,
            )
        coh_mask = coh >= 0
        cs_model_amp = (cs_model * np.conj(cs_model)).real
        num = cs_model_amp - cs._bias(P_noise_ref=ref_P_noise, P_noise_sub=sub_P_noise)
        num[num < 0] = cs_model_amp[num < 0]  # drop bias term is coherence is negative
        den = (fitted_ref_ps_model(cs.freqs) - ref_P_noise) * (fitted_sub_ps_model(cs.freqs) - sub_P_noise)
        model_coh = num / den
        
        axes[0, 2].set_title('Coherence', fontsize='large')
        axes[0, 2].set_ylabel('$\\gamma^2$', fontsize='large')
        axes[0, 2].errorbar(cs.freqs[coh_mask], coh[coh_mask], coh_err[coh_mask], marker='.', ms=2, ls='none', ecolor='grey', elinewidth=1, zorder=0, c='k')
        axes[0, 2].step(cs.freqs, model_coh, where='mid', lw=1, c='r')
        axes[0, 2].set_ylim(-0.05, 1.05)
        
        r = (coh - model_coh) / coh_err
        axes[1, 2].errorbar(cs.freqs, r, np.ones_like(r), ls='none', marker='.', ms=2, ecolor='grey', elinewidth=1, zorder=0, c='k')
        axes[1, 2].axhline(0, c='r', lw=1)
        axes[1, 2].set_ylabel('$\\Delta \\chi$', fontsize='large')
        axes[1, 2].set_xlabel('Frequency [Hz]', fontsize='large')
        
        # compute lags
        plag, plag_err = cs.plags, cs.plag_errs
        plag /= np.pi
        plag_err /= np.pi
        model_plags = np.angle(cs_model) / np.pi
        
        axes[2, 2].set_title('Lag spectrum', fontsize='large')
        axes[2, 2].set_ylabel('Phase lag [$\\pi$]', fontsize='large')
        axes[2, 2].errorbar(cs.freqs[coh_mask], plag[coh_mask], plag_err[coh_mask], marker='.', ms=2, ls='none', ecolor='grey', elinewidth=1, zorder=0, c='k')
        axes[2, 2].step(cs.freqs, model_plags, where='mid', c='r', lw=1)
        axes[2, 2].axhline(0, c='k', lw=.5, zorder=3)
        
        r = (plag.value - model_plags.value) / plag_err.value
        axes[3, 2].errorbar(cs.freqs, r, np.ones_like(r), ls='none', marker='.', ms=2, ecolor='grey', elinewidth=1, zorder=0, c='k')
        axes[3, 2].axhline(0, c='r', lw=1)
        axes[3, 2].set_ylabel('$\\Delta \\chi$', fontsize='large')
        axes[3, 2].set_xlabel('Frequency [Hz]', fontsize='large')
    ...
    def coherence(
        self,
        avg_cs: dict[str, AveragedCrossSpectrum],
        P_noise: dict[str, float],
        scale: Literal['linear', 'semilogx', 'semilogy', 'loglog'] = 'linear',
        save: bool = False,
        return_fig: bool = False,
        ) -> dict[str, dict[str, NDArray]] | tuple[dict[str, dict[str, NDArray]], Figure]:
        """
        Compute the coherence from the given cross-spectra.
        
        Parameters
        ----------
        avg_cs : dict[str, AveragedCrossSpectrum]
            The averaged cross spectra as returned by the `averaged_cross_spectra()` method.
        P_noise : dict[str, float]
            The white noise powers for each band: {key: white noise power}, where key is taken from the keys attribute.
        
        Returns
        -------
        dict[str, dict[str, NDArray]] | tuple[dict[str, dict[str, NDArray]], Figure]
            The coherence and its associated error. If `return_fig=True`, the resulting figure is also returned.
        """
        
        results: dict[str, dict[str, NDArray]] = {}
        
        for key, cs in avg_cs.items():
            key1, key2 = key.split(' x ')
            P_noise1 = P_noise[key1]
            P_noise2 = P_noise[key2]
            
            coh, err = cs.coherence(
                P_noise1=P_noise1,
                P_noise2=P_noise2,
                )
        ...
        if save:
            save_figure(
                fig=fig,
                path=self.out_directory.joinpath(f'plots/{self.prefix}_coherence.pdf'),
                )
        
        if self.show_plots and not return_fig:
            plt.show()
